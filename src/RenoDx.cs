...
public static class RenoDx {
 ...
 public static bool HasHdrAddon(Game g){string dir=Path.GetDirectoryName(g.Exe);return Directory.Exists(dir)&&Directory.GetFiles(dir,"renodx-*.addon64").Any(f=>!Regex.IsMatch(Path.GetFileName(f),"dlss|neural",RegexOptions.IgnoreCase)&&DlssCore.Pe64(f));}
 ...
 public static string State(Game g){string dir=Path.GetDirectoryName(g.Exe);var loaders=Loaders(dir);var addons=Directory.GetFiles(dir,"renodx-*.addon64").Select(Path.GetFileName).ToList();
 ...
 public static CatalogMod ResolveGame(string markdown,string title){
  ...
  foreach(string line in markdown.Split('\n')){
   ...
   foreach(Match link in Regex.Matches(cells[3],@"https://[^\s)<>""]+\.addon64")){
    ...
    if(!allowed||Regex.IsMatch(Path.GetFileName(uri.AbsolutePath),"dlss|neural",RegexOptions.IgnoreCase))continue;
...
public static class ComponentCleanup {
 public static DlssPlan Prepare(Game game,bool reshade){
  ...
  if(reshade){
   ...
   foreach(string path in Directory.GetFiles(root,"renodx-*.addon64")){string n=Path.GetFileName(path);if(!Regex.IsMatch(n,"dlss|neural",RegexOptions.IgnoreCase)&&DlssCore.Pe64(path))add(n);}
...
public static class RenoDxUi {
 ...
 public static Window Create(Window owner,Game game,bool embedded=false){StackPanel body;var d=Dialogs.Create(owner,"HDR · ReShade e RenoDX",out body);d.Width=950;d.FontSize=14;d.MaxHeight=SystemParameters.WorkArea.Height-40;if(embedded)body.Children.Clear();
  ...
  Action<string,Action> button=(label,action)=>{var b=new Button{Content=label,Margin=new Thickness(0,0,8,10)};var buttonStyle=d.TryFindResource(label=="Installa ReShade selezionato"?"Primary":(label.StartsWith("Ripristina")||label.StartsWith("Disinstalla"))?"RestoreAction":label.StartsWith("Catalogo")||label.StartsWith("Importa")?"InfoAction":"Unused") as Style;if(buttonStyle!=null)b.Style=buttonStyle;b.Click+=(s,e)=>{if(IsBusy)return;try{action();}catch(Exception ex){info.Text=ex.Message;}};if(label.StartsWith("Ripristina"))restoreButtons.Children.Add(b);else if(label.StartsWith("Importa"))details.Children.Add(b);else actions.Children.Add(b);};
